...
export const AUTOMATION_FIELD_HELP: Record<string, string> = {
  ...
  "hooks.maxConcurrent":
    "Number of hook agent runs allowed to execute at once, defaulting to one. Hook dispatch holds a non-borrowable reservation inside the shared cron budget, so each additional slot is permanently withheld from cron inner work even while hooks are idle; raise it only when concurrent hook throughput is worth that trade, and expect the value to be clamped so cron keeps at least one slot. Concurrency is per session key: runs sharing one resolved session key still serialize before lane admission, so a width above the number of distinct keys in flight reserves capacity that cannot be used. Raising this above one lets hook runs mutate shared state simultaneously, so only do so when each run is isolated from the others (separate working directory or workspace); widths above one are unsafe for hooks that operate on one shared checkout.",
